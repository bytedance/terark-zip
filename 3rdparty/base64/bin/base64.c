...
enc (FILE *fp)
{
	...
	trk_base64_stream_encode_init(&state, 0);

	while ((nread = fread(buf, 1, BUFSIZE, fp)) > 0) {
		trk_base64_stream_encode(&state, buf, nread, out, &nout);
		if (nout) {
	...
	trk_base64_stream_encode_final(&state, out, &nout);
...
dec (FILE *fp)
{
	...
	trk_base64_stream_decode_init(&state, 0);

	while ((nread = fread(buf, 1, BUFSIZE, fp)) > 0) {
		if (!trk_base64_stream_decode(&state, buf, nread, out, &nout)) {
