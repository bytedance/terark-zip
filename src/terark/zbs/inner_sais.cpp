...
#define sais_index_type int32_t
...
template<typename SAT>
sais_index_type
sais_inner_core(SAT *source, sais_index_type *suf_arr,
  ...
  if(name < m) {
    ...
    if(sais_inner_core(sub_source, suf_arr, newfs, m, name) != 0) {
  ...
  induceSA(source, suf_arr, alphabet, bucket, size, sigma);
  if(flags & (1 | 4)) { SAIS_MYFREE(alphabet, sigma, sais_index_type); }
  if(flags & 2) { SAIS_MYFREE(bucket, sigma, sais_index_type); }
...
sufarr_inducedsort(const unsigned char *source, int *suf_arr, int size, int sigma = UCHAR_SIZE) {
  ...
  return sais_inner_core(source, suf_arr, 0, size, sigma);
}
...
sufarr_inducedsort_int(const int *source, int *suf_arr, int n, int k) {
  ...
  return sais_inner_core(source, suf_arr, 0, n, k);
}

int
sufarr_inducedsort_int_recursion(const int *source, int *suf_arr, int n, int k){
  if(n <= 1) { if(n == 1) { suf_arr[0] = 0; } return 0; }
  return sais_inner_core(source, suf_arr, 0, n, k);
}
